<template>
  <div v-if="notes.length" class="small">
...
export default {
  ...
  methods: {
    ...
    result_per_gen() {
        ...
        this.positives().filter(function(item) {
            ...
            result[JSON.parse(item.content).gen] = ((result[JSON.parse(item.content).gen] *100) / notes_long).toFixed(2)
            return result
        })
        return result
    },
